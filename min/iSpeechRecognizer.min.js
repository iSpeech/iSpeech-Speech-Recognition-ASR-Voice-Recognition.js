...
iSpeechRecognizer=function(params){this.commands=[];this.aliasList={};this.optionalCommands={};this.endpoint="wss://malcom.ispeech.org:8181/";params=params||{};this.apiKey=params.apiKey||"developerdemokeydeveloperdemokey";this.onResponse=params.onResponse||this.onResponse;this.silenceDetection=params.silenceDetection||true;this.workerLoc=params.workerLoc||'iSpeechWorker.min.js';window.navigator=window.navigator||{};navigator.getUserMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||null;if(navigator.getUserMedia===null){this.onResponse({result:'error',code:10001,message:'Browser not supported'});return;}
...
return;this.state=iSpeechRecognizer.IDLE;this.worker.postMessage({command:'stop'});this.worker.terminate();this.audioContext.close();this.mediaStream.stop();this.node.disconnect();this.node.onaudioprocess=function(){};}
